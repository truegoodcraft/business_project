...
- Primary authority basis: `core/version.py`, `docs/TESTING_POLICY.md`, `scripts/validate_version_governance.py`, `scripts/validate_change_trace.py`, `scripts/build_core.ps1`, `scripts/release-check.ps1`, `BUS-Core.spec`, `core/api/routes/update.py`, `core/services/update.py`, `.github/workflows/ci.yml`, `.github/workflows/governance-guard.yml`, `.github/workflows/security-audit.yml`, `.github/workflows/release-mirror.yml`, `.github/workflows/publish-image.yml`, `.github/workflows/publish-wiki.yml`.
...
| Risk-based test selection | `docs/TESTING_POLICY.md` | Local and agent verification scope | Canonical policy | T0-T2 changes use proportional local targets; T3 and release candidates use the full regression chain. Critical-boundary rules override file counts, and CI remains independently governed. |
| Active cross-platform CI | `.github/workflows/ci.yml` | Push, pull request, and manual CI evidence | Canonical guard | Runs locked Linux 3.12 and Windows 3.11 compilation/full pytest plus bounded Linux Atheris harnesses; proportional local selection does not reduce this gate. |
...
10. `scripts/release-check.ps1` is the governed clean Windows gate: it creates a Python 3.11 environment, may contact configured package indexes while installing hash-locked runtime/test graphs, runs compile/tests/governance/isolated local launch smoke, and performs an unsigned build by default or the certificate/timestamp-using signed bundle path only with its separately approved release option.
    - This is release evidence, not the default edit loop. The isolated source smoke proves business flows; the build path separately proves frozen-executable startup and artifact structure.
