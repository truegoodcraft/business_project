...
BUS Core Self-Managed remains usable on local infrastructure without a hosted account, subscription, or forced cloud dependency. On packaged Windows installs, business data and configuration live under `%LOCALAPPDATA%\BUSCore`; in Docker, the main database is normally under the operator-mounted `/data` directory. BUS Core v1.4.2 supports documented optional update checks and a disclosed, optional, fail-open product client that cannot accept business content.
...
Use official project release locations and read [Updates and Releases](Updates-and-Releases.md). Core has no silent background auto-update. Lighthouse 1.27.0 and migration 0015 are deployed and production-verified. The BUS Core client is disclosed, controllable, meaningfully optional, non-blocking, restricted by that live verified contract, and unable to carry business content.
