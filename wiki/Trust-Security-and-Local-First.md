...
BUS Core Self-Managed remains usable on local infrastructure without a hosted account, subscription, or forced cloud dependency. On packaged Windows installs, business data and configuration live under `%LOCALAPPDATA%\BUSCore`; in Docker, the main database is normally under the operator-mounted `/data` directory. BUS Core v1.4.1 supports documented optional update checks and a disclosed, optional, fail-open product client that cannot accept business content.
